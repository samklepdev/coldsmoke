...
# Coldsmoke Plan 4 — Admin Foundation & Orders

Plan: docs/superpowers/plans/2026-09-20-admin-orders.md
Spec: docs/superpowers/specs/2026-09-20-admin-orders-design.md
Branch: feat/admin-orders (from main @ 30e68bb)
Started: 2026-09-20

## Pre-flight fixes (applied to plan before execution, commit c40e5ce)
- Button has no "secondary" variant (primary | outline | quiet). Tasks 9 and
  13 used it and would not have typechecked. Now "outline".
- --bright / --muted are not real tokens. Correct names --text-bright /
  --text-dim, defined in src/styles/tokens.css, NOT globals.css. Fixed in
  Tasks 3, 7, 8 CSS.
- Also fixed during plan self-review: Task 12 referenced invented webhook test
  helpers (real ones are post() and placeOrder()); Task 13 said "reuse Task 9's
  mocks" which a fresh subagent cannot see, so the test file is now inlined.

## Notes for all tasks
- src/lib/business.ts has an UNCOMMITTED user edit (filling in legal name,
  address, support email). Implementers must stage only their own paths.
  Never `git add -A`.
- No verified Resend domain, so every send fails with a 403. This is expected;
  the design surfaces it rather than hiding it. Do not "fix" it.
- Test DB: npm run db:test:up before running vitest.

## Progress
Task 1: complete (commits c40e5ce..2f48bbd, review clean first pass)
  - requireAdminUser: redirect signed-out, notFound() non-admin AND null role.
  - Reviewer confirmed the tests distinguish notFound() from redirect() (mocks
    throw different strings + negative assertions), so they test the security
    property rather than merely "it throws".
  - Minor deferred to final review: the `next ? /sign-in?next=… : /sign-in`
    construction is duplicated verbatim from requireSessionUser rather than
    delegating. Plan-specified; refactor candidate.
Task 2: complete (commits 2f48bbd..f7d8134, review clean first pass)
  - promote-admin. Implementer correctly added `import "dotenv/config"`, which
    the plan's code block omitted; seed.ts and migrate.ts both need it and the
    script cannot read DATABASE_URL without it. Plan updated to match (f7d8134)
    rather than weakening plan-drift, which compares Create blocks byte-for-byte.
    Reviewer diffed plan block vs shipped file: IDENTICAL.
  - Minor deferred: process.exit() could in theory truncate piped stdout.
    Pre-existing pattern in seed.ts and migrate.ts; not introduced here.

Controller fix between Tasks 2 and 3 (commit below):
  - e2e/accounts.spec.ts had TWO FAILING TESTS on main, caused by our own
    earlier sign-up delivery-signal work. They asserted getByText("Check"),
    which only appears when the confirmation email is accepted. Fixed by
    asserting the address both branches echo instead. 6/6 pass now.
  - Lesson: vitest+tsc+lint were run on that work, Playwright was not, and it
    was merged to main. Run playwright before merging anything touching a
    user-facing flow.
  - Plan Task 3 step corrected: playwright.config.ts has a webServer with
    reuseExistingServer; implementers must not start dev by hand.

Controller note during Task 3 — STALE ARTIFACT TRAP:
  .superpowers/sdd/ is shared across plans and task-N-report.md filenames
  COLLIDE. Eight reports from the 2026-09-19 storefront plan (task-4 through
  task-11) were still sitting there with the exact names this plan will use.
  If an implementer ever failed to write its report, the reviewer would have
  been handed a different plan's report and reviewed the wrong work while
  reporting success. Archived to .superpowers/sdd/archive-pre-admin-plan/.
  task-3-report.md was left in place only because its implementer was
  mid-write; check its mtime is from THIS session before reviewing.
  Rule going forward: before dispatching any reviewer, confirm the report
  file's mtime is newer than the task's base commit.
Task 3: complete (commits b70871f..55a5b9d + plan note, review clean first pass)
  - /admin shell, role gate in the layout, e2e/admin.spec.ts.
  - DEVIATION 1 (accepted, verified): implementer added a placeholder
    src/app/(admin)/admin/orders/page.tsx. Next matches a route only where a
    page.tsx exists; without one the global 404 is served WITHOUT running any
    ancestor layout, so the gate would have been "tested" by a request that
    never reached it. Task 7 overwrites this file.
  - DEVIATION 2 (accepted, verified): brief's e2e asserted a 404 STATUS CODE.
    IMPORTANT NEXT 16 FACT: notFound() thrown after an await streams, and a
    streamed response cannot have its status changed -- it returns 200 with
    not-found BODY. Confirmed independently by the reviewer in
    node_modules/next/dist/docs (loading.md "Status Codes", not-found.md).
    Never assert a 404 status code for notFound() after an await.
    Test now asserts not-found content + no redirect + absence of admin nav.
    Reviewer traced all three tripwires: each fails if the gate is removed or
    swapped for a redirect. Not vacuous.
  - Also: sign-up cannot establish a session while requireEmailVerification is
    on, so e2e flips emailVerified by direct DB write. First spec in e2e/ to
    touch the DB.
  - Minor deferred to final review: that DB-write pattern deserves a shared
    helper if a second spec needs it.

USER-VISIBLE ISSUE (not ours, reported to user, awaiting their call):
  src/lib/business.test.ts FAILS against the user's uncommitted business.ts
  edit. Not a defect -- the placeholder guard is designed to fail when a
  placeholder is filled in, until its allowlist is updated to []. Also flagged
  to the user: isPending() only detects BRACKETED markers, so their
  addressLine1 "123 Main Street" passes the guard while reading like a
  stand-in, and it renders into Terms and Privacy.
Task 4: complete (commit 3f5d838, review clean first pass, no findings)
  - src/lib/orders/errors.ts: OrderNotFoundError, OrderNotFulfillableError,
    OrderNotRefundableError. No import of index.ts, keeping the dependency
    one-way so fulfill.ts/refund.ts can use both without a cycle.
Controller pre-verification (so later tasks don't discover these the hard way):
  - revalidatePath(path, type?) EXISTS in Next 16, no deprecation notice.
    Tasks 9 and 13 depend on it. Checked in node_modules/next/dist/docs.
  - pgEnum(...).enumValues returns the string array (drizzle 0.45.2).
    Task 7's status filter depends on it.
  - testDb() -> { db, truncate(), close() }; truncate() covers orders and
    order_items. formatCents is exported from @/lib/money.
Task 5: complete (commits 3f5d838..fc0e633 + plan fix 64b6167)
  *** FORMAL REVIEW WAS STOPPED BY THE USER. Replaced by controller
  verification, which is NARROWER than a task review: 5/5 tests pass,
  WHERE-clause guard present at fulfill.ts:42, fulfillOrder NOT re-exported
  from index.ts (no cycle). NOT independently checked: whether the
  double-fulfil test proves the WHERE guard or only the in-transaction
  status check, and whether any assertion is absence-only. Flag both to the
  FINAL whole-branch review. ***
  - DEVIATION (accepted, high value): implementer added
    vi.mock("@/lib/db/client") to the test. Without it, fixtures write to the
    TEST db (54329) while fulfillOrder reads the DEV db (54328) -- the tests
    would have been green while exercising nothing. Applied preemptively to
    Tasks 7 and 11 in commit 64b6167, and briefs 6-14 REGENERATED from the
    corrected plan (they were snapshots of the old text).
  - DEVIATION: `as const` on ADDRESS.country; Address.country is the literal
    "US", not string. markPaid.test.ts already does this.
Task 6: complete (commit 0a18ade, review clean first pass, no findings)
  - ShippingConfirmation.tsx + shipping.ts returning { delivered }.
  - Reviewer confirmed the key property: deleting the `if (error)` branch
    makes shipping.test.ts fail, so the tests genuinely distinguish "Resend
    returned an error object" from "Resend accepted it". That is the exact
    bug this task exists to prevent.
  - No absence-only assertions. Template styles byte-identical to
    OrderConfirmation.tsx, so the two emails stay one family.
  - Minor deferred to final review: shipping.ts and auth.ts send() have
    near-duplicate try/catch/log bodies. Two call sites; unifying now would
    be premature.
Task 7: IMPORTANT DEFECT FOUND BY REVIEW, fix in flight.
  - Spec passed, but an all-digit query above int4 max (2147483647) threw
    Postgres 22003 -> unhandled rejection in a Server Component -> generic
    error page. Reviewer flagged it as reasoned-not-observed; controller
    CONFIRMED it by running listOrdersForAdmin directly:
      q=1042 -> OK; q=99999999999 -> THREW; q=2147483648 -> THREW.
  - Root cause is the PLAN's code: it required untrusted `status` to degrade
    to "no filter" but never extended the rule to `query`, from the same URL.
  - Fix semantics (deliberately different from status): out-of-range number
    means ZERO RESULTS, not "no filter". Showing every order to someone who
    searched for one specific order looks like an answer and is worse.
  - Reviewer also confirmed: drizzle parameterises the ILIKE (no injection;
    % and _ only widen the match), and count(*) shares the same `where` as
    the page query so pagination cannot lie.

Task 8: complete (commit 17fb0fa, Spec OK, quality Approved)
  - Read-only order detail page. No tests by design.
  - IMPORTANT (from the PLAN's own code, fix pending): detail page renders
    raw .toISOString() with milliseconds, while the sibling list page renders
    .slice(0,10). Visually inconsistent side by side.
  - Minor (deferred): items table has no thead/th, unlike the list page.
  - Minor (verified safe, not a bug): carrier/trackingNumber are independently
    nullable, so the fulfilment block could in principle render blanks --
    but fulfillOrder is the ONLY writer of fulfilledAt and always sets all
    three together. Safe only while that invariant holds.
  - Implementer created a throwaway admin account in the DEV db to verify in
    browser, then deleted it. Controller verified: no leftover accounts, 0
    orphaned sessions, bayou.city.labs.hou@gmail.com still the only admin.
Tasks 9-14: complete. Implemented DIRECTLY by the controller, not subagents --
the user killed the Task 9 implementer and its reviewer and asked to keep
going, so tasks 9-14 have NO independent task review. Commits:
  87bb6ce date/table fixes (Task 8 review findings)
  5c493a7 Task 9  fulfilment action + form
  a7832a7 Task 10 refund idempotency key
  928216d Task 11 refundOrder + recordRefund (cumulative rule pinned)
  86b7449 Task 12 charge.refunded webhook branch
  74e1e58 Task 13 refund button
  70e3da1 Task 14 plan-drift widened + guard hole fixed

*** FOR THE FINAL WHOLE-BRANCH REVIEW -- carry these forward: ***
  1. Tasks 9-14 were never independently reviewed (see above). Review them
     fresh rather than assuming a gate already passed.
  2. Task 5 open questions from its stopped review: does the double-fulfil
     test prove the WHERE-clause guard or only the in-transaction status
     check? Any absence-only assertions?
  3. Deferred Minors: requireAdminUser duplicates requireSessionUser's
     sign-in URL construction; shipping.ts and auth.ts send() near-duplicate
     try/catch bodies; promote-admin's process.exit could truncate piped
     stdout (pre-existing pattern).
  4. Task 8 invariant worth re-checking: the detail page's fulfilment block
     is safe ONLY because fulfillOrder always writes carrier + trackingNumber
     + fulfilledAt together. Nothing enforces that beyond that one function.

plan-drift found TWO classes of its own blind spots when widened:
  - "Append to `path`:" headings that WRAP to a second line are silently
    skipped (regex is [^\n]*). Four of mine wrapped. Keep them on one line.
  - parseBlocks did code.trim(), stripping the FIRST line's indentation, so
    any appended fragment starting indented was unverifiable by construction.
    Fixed to trailing-only. Guarded blocks went 76 -> 108.

Final verification on 70e3da1:
  vitest 449/450 (only failure: business.test.ts, the user's uncommitted
  business.ts edit -- NOT this branch), tsc clean, lint clean,
  playwright 18/18.
  NOTE: the test DB container exits on its own periodically; if a whole
  swathe of DB tests fail with ECONNREFUSED ::1:54329, run npm run db:test:up
  and re-run before believing the failures.

## Final whole-branch review (opus) — verdict: changes needed. ALL FIXED in 4fc4e14.

CRITICAL (real money bug, was shipping):
  recordRefund SET refundedCents unconditionally. Stripe does not guarantee
  event order, so 1000-then-2500 delivered backwards left refundedCents=1000
  on an order the 2500 event had already marked "refunded" -- and refundOrder
  then refuses it (status not paid/fulfilled), so it was unremediable. The
  ledger cannot catch this: different event ids, both genuinely new.
  FIX: monotonic set (Math.max), status derived from the applied value.

IMPORTANT, all fixed:
  - recordRefund had no status precondition: charge.refunded arriving before
    payment_intent.succeeded wrote "refunded" over "pending", stranding the
    stock reservation forever and making the retried succeeded event throw
    StrandedPaymentError until Stripe gave up. Now refuses -> non-2xx -> retry.
  - resendShippingAction never checked the order shipped. A POST against a
    paid order emailed "on its way" with blank carrier/tracking.
  - The admin gate was unprovable: action tests mocked @/lib/auth/session
    wholesale, so deleting requireAdminUser left the suite green. Mock is now
    a spy; the call is asserted for all three actions.
  - e2e had no positive admin case (spec §9 asked for one). Both existing
    tests would pass with the admin shell broken for everyone.

ANSWERED (Task 5's stopped-review questions):
  - The double-fulfil test is SEQUENTIAL, so it passes at the in-transaction
    status check. Deleting eq(orders.status,"paid") from fulfill.ts:42 leaves
    it green. The WHERE guard and its "paid, then changed" branch remain
    untested -- only a genuinely concurrent test reaches them. STILL OPEN.
  - Absence-only assertions: one that mattered (bare toThrow() on the
    no-order-for-payment path) -- fixed to assert the error name.

STILL OPEN, deliberately:
  - Double refund after a lost webhook: Stripe idempotency keys expire after
    24h, so if charge.refunded never arrives, refundedCents stays 0, the
    button stays live, and a click >24h later issues a REAL second refund.
    Nothing records that a refund was requested. Accepted risk; would need a
    refund_requested_at column (a migration) to close.
  - The WHERE-clause guard in fulfillOrder is untested (above).
  - Minors not fixed: requireAdminUser duplicates requireSessionUser's
    sign-in URL construction; promote-admin's process.exit could truncate
    piped stdout. Reviewer judged the shipping.ts/auth.ts send() duplication
    should NOT be unified -- spec §6 names three deliberately different
    email-failure policies and a shared helper would couple them.

Verification on 4fc4e14: vitest 455/456 (only failure is business.test.ts,
the user's uncommitted business.ts edit), tsc clean, lint clean,
playwright 18 passed / 1 skipped -- the skip is checkout's real-payment test,
which self-skips when no `stripe listen` forwards to the webhook. Environmental.
